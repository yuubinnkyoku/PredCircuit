...
Active pilot stage. The code now supports real FlyVis retinotopy, temporal local-PC learning, exact-gradient controls, constrained topology nulls, target-alignment tests, scaling experiments, and hardware-locality estimates. Residual-MLP credit work now includes PC-ALM/ePC references, dual-leak fixed-budget windows, mixed-precision/RTL cost models, and magnitude-window diagnostics that separate credit direction from credit scale (`docs/magnitude-window-crossline.md`, `results/magnitude_window_holdout_2026-09-18.md`). The next goal is to separate generic topology/task alignment from a genuinely local-credit-specific effect, to determine why the current local rule does not automatically scale from extent 1 to extent 2, and to bank the hardware path on magnitude-stabilized dual dynamics rather than pure sPC iteration count.
