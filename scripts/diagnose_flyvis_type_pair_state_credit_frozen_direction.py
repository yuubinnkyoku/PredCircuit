...
def run_seed(
    *,
    ...
    names = tuple(
        _condition_name(mode, condition) for mode in MODES for condition in STATE_CREDIT_CONDITIONS
    )
    models = {
        ...
        if BURST_START <= epoch < BURST_END:
            ...
            if epoch < BURST_START:
                ...
                for name in names:
                    ...
                    source_cosine_to_current_donor[name] += _cosine(applied_edge, source_reference)
            ...
            rows.append(
                {
                    ...
                    "mean_postburst_own_credit_relative_distance_to_local": (
                        own_credit_relative_distance[name] / credit_count if credit_count else 0.0
                    ),
                    ...
                    "mean_postburst_source_cosine_to_current_donor": (
                        source_cosine_to_current_donor[name] / credit_count if credit_count else 1.0
                    ),
