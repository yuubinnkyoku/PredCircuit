...
HORIZONS = (60, 80, 100)
EVAL_REPS = 4
EVAL_JITTER_BASE = 4_700_000
INIT_MODES = ("random", "biological_strength")
RULES = ("local", "standard", "threshold")
...
def _threshold_direction(
    ...
    for _, _, indices in named_groups:
        ...
        suppress = ratio >= MEAN_DOMINANCE_THRESHOLD
...
def run_seed(*, seed: int, learning_rate: float, max_update: float) -> pd.DataFrame:
    ...
    for step in range(1, max(HORIZONS) + 1):
        ...
        for mode in INIT_MODES:
            ...
            direction, diagnostics = _threshold_direction(raw_edge, named_groups)
            apply_local_credit(
                threshold,
                direction,
                raw_bias,
                    ...
                    rows.append(
                        {
                            ...
                            "eval_rep": eval_rep,
                            "rule": rule,
                            "mean_dominance_threshold": MEAN_DOMINANCE_THRESHOLD,
                            **metrics,
                            **diagnostics,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fresh hold-out of a conservative topology-agnostic rule that reduces the "
            "extra type-pair mean gain from 3 to 1 only when ||m||/||r|| >= 0.5."
        )
    )
