...
def main() -> None:
    ...
    files = sorted(args.input.rglob("pcalm_accumulator_precision_seed*.csv"))
    if not files:
        raise FileNotFoundError("no accumulator precision seed CSVs found")
    frame = pd.concat((pd.read_csv(path) for path in files), ignore_index=True)
    summary = (
        frame.groupby("accumulator_precision", sort=False)
        .agg(
            seeds=("seed", "nunique"),
