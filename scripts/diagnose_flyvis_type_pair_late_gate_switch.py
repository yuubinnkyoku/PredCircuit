...
HORIZONS = (100, 140, 160, 180, 200)
SWITCH_EPOCHS = (0, 100, 140, 160, 180, 200)  # 200 = never gate (pure standard)
EVAL_REPS = 4
EVAL_JITTER_BASE = 9_500_000
...
def run_seed(*, seed: int, learning_rate: float, max_update: float) -> pd.DataFrame:
    ...
    models = {sw: copy.deepcopy(base) for sw in SWITCH_EPOCHS}
    rows: list[dict[str, float | int | bool | str]] = []

    for step in range(1, max(HORIZONS) + 1):
        epoch = step - 1
        for sw in SWITCH_EPOCHS:
            model = models[sw]
        ...
        for sw in SWITCH_EPOCHS:
            ...
            for eval_rep in range(EVAL_REPS):
                ...
                # use switch=200 (pure standard) as the paired local_readout reference
                # actually use a fresh local-style readout from this model itself
                readout, classes = _heldout_readout(model, circuit, jitter_seed=jitter_seed)
                # compute metrics without a separate local reference
                ...
                finite = (
                    ...
                    and all(
                        math.isfinite(v) for v in (ce, acc, hard_margin, soft_margin, weight_norm)
                    )
                )
                rows.append(
                    {
                        ...
                        "gated_fraction": min(max(step - sw, 0), step) / step,
                        "finite": finite,
                    }
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Causal switch experiment: train ungated 4m+r until a switch epoch, then apply "
            "the rho05 gate for the remainder, to locate when gating becomes harmful."
        )
    )
