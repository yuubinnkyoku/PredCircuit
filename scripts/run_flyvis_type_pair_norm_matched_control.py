...
def match_norm(candidate: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
...
def run_seed(
    *,
    ...
    for epoch in range(epochs):
        ...
        elif rule == "type_pair_norm_matched":
        ...
        elif rule == "shuffled_norm_matched":
            mixed = linear_mix_direction(
                local_edge,
                shuffled_groups,
                coarse_gain=4.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare local credit with norm-matched biological and shuffled 4m+r credit"
    )
    parser.add_argument("--seed", type=int, required=True)
    parser.add_argument(
        "--rule",
        choices=("local", "type_pair_norm_matched", "shuffled_norm_matched"),
        required=True,
    )
