...
HORIZONS = (60, 80, 100)
EVAL_REPS = 4
EVAL_JITTER_BASE = 3_500_000
RULES = ("local", "type", "l1_only", "core_l1", "retina_l1", "mi9_only")
RETINA_SOURCES = {f"R{i}" for i in range(1, 9)}
CORE_L1_SOURCES = {"Mi1", "Tm3"}
...
def _selective_direction(
    ...
    *,
    ...
    for source_type, target_type, indices in named_groups:
        selected = target_type == target and (sources is None or source_type in sources)
        if selected:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether L1-targeted type-pair shared credit causally explains the "
            "biological-strength full type-pair advantage"
        )
    )
