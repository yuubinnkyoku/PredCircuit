...
def main() -> None:
    ...
    model = ResidualMLP(depth=depth, width=width, input_dim=input_dim, output_dim=output_dim, activation="relu", seed=args.seed + 1000 * width + depth)
    ...
    for update in range(updates + 1):
        ...
        if update in {0, 4, 8, 16, 24}:
            bp = method_grad(model, x, y, Schedule("bp", budget=0), state_lr=args.state_lr, rho=1.0)
            ...
            for layer, (pg, bg) in enumerate(zip(pc, bp, strict=True)):
                rows.append({
                    ...
                    "layer_grad_norm_ratio_bp": float(pg.norm() / bg.norm().clamp_min(torch.finfo(bg.dtype).eps)),
