...
HORIZONS = (40, 80, 120, 160, 180, 200)
EVAL_REPS = 4
EVAL_JITTER_BASE = 10_940_000


def _credit_geometry(
    ...
    *,
    ...
    for _, _, indices in named_groups:
        shared[indices] = raw_edge[indices].mean()
    residual = raw_edge - shared
    direction = residual + coefficient * shared
    preclip_update = learning_rate * direction
    clipped_update = preclip_update.clone()
    if max_update > 0.0:
        clipped_update.clamp_(-max_update, max_update)
        clip_fraction = float((preclip_update.abs() >= max_update).float().mean())
    else:
        clip_fraction = 0.0

    weight = model.weight.detach()
    update_norm = torch.linalg.vector_norm(clipped_update)
    weight_norm = torch.linalg.vector_norm(weight)
    denominator = (weight_norm * update_norm).clamp_min(1e-30)
    radial_cosine = float(torch.dot(weight, clipped_update) / denominator)
    radial_projection = float(torch.dot(weight, clipped_update) / weight_norm.clamp_min(1e-30))

    return {
        "raw_credit_norm": float(torch.linalg.vector_norm(raw_edge)),
        "shared_credit_norm": float(torch.linalg.vector_norm(shared)),
        "residual_credit_norm": float(torch.linalg.vector_norm(residual)),
        "direction_norm": float(torch.linalg.vector_norm(direction)),
        "preclip_update_norm": float(torch.linalg.vector_norm(preclip_update)),
        "clipped_update_norm": float(update_norm),
        "clip_fraction": clip_fraction,
        "radial_update_cosine": radial_cosine,
        "radial_update_projection": radial_projection,
    }
...
def run_seed(
    *,
    ...
    last_geometry: dict[tuple[float, float], dict[str, float]] = {}
    ...
    for step in range(1, max(horizons) + 1):
        ...
        for scale in init_scales:
            for coefficient in coefficients:
                ...
                last_geometry[(scale, coefficient)] = _credit_geometry(
                    model,
                    raw_edge,
                    named_groups,
                ...
                mean_abs_update, max_abs_update = apply_local_credit(
                    model,
                    direction,
                    raw_bias,
                    learning_rate=learning_rate,
                ...
                last_geometry[(scale, coefficient)]["mean_abs_update"] = mean_abs_update
                last_geometry[(scale, coefficient)]["max_abs_update"] = max_abs_update
        ...
        for scale in init_scales:
            for coefficient in coefficients:
                model = models[(scale, coefficient)]
                geometry = last_geometry[(scale, coefficient)]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Trace local shared/residual credit and clipped-update geometry across the "
            "fixed-coefficient stability boundary."
        )
    )
