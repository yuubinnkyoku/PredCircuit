...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Width-64 PC-ALM dual-leak under official vs project state_lr (FP32)."
    )
    p.add_argument("--seed", type=int, required=True)
    p.add_argument("--depth", type=int, default=32)
    p.add_argument("--width", type=int, default=64)
    p.add_argument("--batch-size", type=int, default=4)
    p.add_argument("--state-lrs", default="0.234285,0.25")
    ...
    model_seed = a.seed + 1000 * a.width + a.depth
    data_seed = a.seed + 10_000 + 1000 * a.width + a.depth
    model = ResidualMLP(
        depth=a.depth,
        width=a.width,
        input_dim=8,
                ...
                rows.append(
                    {
                        ...
                        "state_lr_label": (
                            "sakana_depth32"
                            if abs(state_lr - 0.234285) < 1e-6
                            else f"project_{state_lr:g}"
                        ),
                        "dual_leak": dual_leak,
                        "budget": budget,
