...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Probe compressibility of deep residual credit transport."
    )
    ...
    p.add_argument(
        "--out", type=Path, default=Path("results/generated/credit_jacobian_rank_probe.csv")
    )
    args = p.parse_args()
            ...
            rows.append(
                {
                    ...
                    "approx": "diag",
                    "rank": 0,
                    "cosine": cosine(diag, exact),
                    "relative_error": float((diag - exact).norm() / (exact.norm() + 1e-30)),
    ...
    print(
        frame.groupby("approx")[["cosine", "relative_error", "norm_ratio"]]
        .agg(["mean", "median"])
        .to_string()
    )
