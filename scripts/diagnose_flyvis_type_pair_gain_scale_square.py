...
HORIZONS = (160, 200)
RULES = ("local", "standard", "sq32", "sq40")
EVAL_REPS = 4
EVAL_JITTER_BASE = 10_910_000
...
def _shared_coefficient(rule: str, init_scale: float) -> float:
    ...
    if rule == "standard":
        return 4.0
    if rule == "sq32":
        return 0.032 / (init_scale * init_scale)
    if rule == "sq40":
        return 0.040 / (init_scale * init_scale)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether late shared-credit stability is better normalized by the "
            "shared coefficient times biological initialization scale squared."
        )
    )
