...
RULES = ("local", "type", "mi9_all", "mi9_top5", "mi9_rest")
TOP5_MI9_PAIRS = frozenset(
    {
        ("R7", "Mi9"),
        ("R8", "Mi9"),
        ("T4a", "Mi9"),
...
def _selective_direction(
    ...
    for source_type, target_type, indices in named_groups:
        if (source_type, target_type) in selected_pairs:
...
def run_seed(
    *,
    ...
    all_mi9_pairs = frozenset(
        (source_type, target_type)
        for source_type, target_type, _ in named_groups
        if target_type == "Mi9"
    )
    missing = TOP5_MI9_PAIRS - all_mi9_pairs
    if missing:
        raise RuntimeError(f"expected Mi9 top-five pairs not present: {sorted(missing)}")
    rest_mi9_pairs = all_mi9_pairs - TOP5_MI9_PAIRS
    ...
    for step in range(1, max(HORIZONS) + 1):
        ...
        for rule in ("type", "mi9_all", "mi9_top5", "mi9_rest"):
            model = models[rule]
            raw_edge, raw_bias = credit(model, circuit, seed=seed, epoch=epoch)
            if rule == "type":
                direction = _mixed_direction(raw_edge, groups)
            elif rule == "mi9_all":
                direction = _selective_direction(raw_edge, named_groups, all_mi9_pairs)
            elif rule == "mi9_top5":
                direction = _selective_direction(raw_edge, named_groups, TOP5_MI9_PAIRS)
            else:
                direction = _selective_direction(raw_edge, named_groups, rest_mi9_pairs)
                ...
                rows.append(
                    {
                        ...
                        "eval_rep": eval_rep,
                        "rule": rule,
                        "mi9_group_count": len(all_mi9_pairs),
                        "top5_group_count": len(TOP5_MI9_PAIRS),
                        "rest_group_count": len(rest_mi9_pairs),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether the five Mi9-targeting type pairs with the largest preregistered "
            "cross-curvature contributions capture the long-horizon shared-credit benefit"
        )
    )
