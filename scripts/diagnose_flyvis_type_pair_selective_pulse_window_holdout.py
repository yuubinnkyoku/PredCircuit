...
INIT_SCALE = 0.115
WINDOWS = ((80, 100), (100, 120), (120, 140), (140, 160), (160, 180), (180, 200))
END_EPOCH = 200
EVAL_REPS = 4
EVAL_JITTER_BASE = 151_000_000
...
def _branch_pulse_to_end(
    ...
    *,
    ...
    steps = window_end - window_start
    for epoch in range(window_start, window_end):
        raw_edge, raw_bias = credit(model, circuit, seed=seed, epoch=epoch)
        direction = _standard_direction(raw_edge, groups)
        if rule == "standard":
            apply_local_credit(
...
def run_seed(*, seed: int, learning_rate: float = 160.0, max_update: float = 0.05) -> pd.DataFrame:
    ...
    for window_start, _ in WINDOWS:
        _advance_standard(
            ...
            seed=seed,
            start_epoch=trained,
            end_epoch=window_start,
            learning_rate=learning_rate,
            max_update=max_update,
        )
        trained = window_start
        snapshots[window_start] = copy.deepcopy(base)

    for window_start, window_end in WINDOWS:
        snapshot = snapshots[window_start]
        models = {rule: copy.deepcopy(snapshot) for rule in RULES}
        diagnostics = {
            rule: _branch_pulse_to_end(
                model,
                circuit,
                groups,
                rule=rule,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fresh common-state pulse-window hold-out localizing which epochs make "
            "type-pair selective attenuation outperform globally L2-matched attenuation."
        )
    )
