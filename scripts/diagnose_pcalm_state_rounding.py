...
def q_stochastic(x: torch.Tensor, mode: str, gen: torch.Generator) -> tuple[torch.Tensor, int]:
    ...
    prob = v - lower
    q = (
        lower + (torch.rand(prob.shape, generator=gen, device=prob.device) < prob).to(prob.dtype)
    ) * step
    return q.detach(), sat
...
def run(model, x, y, *, rounding: str, seed: int, budget: int, state_lr: float, dual_leak: float):
    ...
    finite = True
    _, state_lo, state_hi = fixed_params("fixed15_i3")
    ...
    for outer_ix in range(budget):
        ...
        for z, grad, err in zip(variables, raw_grads, feedback, strict=True):
            ...
            new_feedback.append(
                (target.clamp(state_lo, state_hi) - q_state).detach()
                if rounding == "error_feedback"
                else torch.zeros_like(q_state)
            )
        ...
        finite = finite and all(
            bool(torch.isfinite(t).all()) for t in [*free, *duals_after, *feedback]
        )
...
def main() -> None:
    ...
    rows = []
    trials = [("nearest", 0), ("error_feedback", 0)] + [
        ("stochastic", i) for i in range(a.stochastic_repeats)
    ]
