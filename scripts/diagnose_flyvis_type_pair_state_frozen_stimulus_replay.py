...
MODES = ("live", "state_frozen_replay")
CONDITIONS = (
    "type_donor",
    "shuffle_donor",
...
def _copy_model(model: PredictiveCodingGraph, circuit) -> PredictiveCodingGraph:
    copied = PredictiveCodingGraph(circuit.graph, seed=0, init_scale=0.08)
    copied.weight.copy_(model.weight)
    copied.bias.copy_(model.bias)
    return copied
...
def run_seed(
    *,
    ...
    frozen_type_state: PredictiveCodingGraph | None = None
    frozen_shuffle_state: PredictiveCodingGraph | None = None
    ...
    source_relative_distance_to_live = {name: 0.0 for name in names}
    ...
    for epoch in range(100):
        ...
        if epoch < BURST_START:
            ...
            for name in names:
                ...
                current_edge = credit(models[name], circuit, seed=seed, epoch=epoch)[0]
                groups = biological_groups if _is_type_state(condition) else shuffled_groups
                ...
                if _is_type_state(condition):
                    treatment_edges[name] = match_norm(mixed, current_edge)
                else:
            ...
            if frozen_type_state is None:
                frozen_type_state = _copy_model(
                    models[_name("state_frozen_replay", "type_donor")], circuit
                )
                frozen_shuffle_state = _copy_model(
                    models[_name("state_frozen_replay", "shuffle_donor")], circuit
                )
            assert frozen_shuffle_state is not None
            ...
            replay_type_raw = credit(frozen_type_state, circuit, seed=seed, epoch=epoch)[0]
            replay_shuffle_raw = credit(frozen_shuffle_state, circuit, seed=seed, epoch=epoch)[0]
            replay_type = match_norm(replay_type_raw, live_type)
            replay_shuffle = match_norm(replay_shuffle_raw, live_shuffle)
            sources = {
                "live": {"type": live_type, "shuffle": live_shuffle},
                "state_frozen_replay": {"type": replay_type, "shuffle": replay_shuffle},
            }
            ...
            for name in names:
                ...
                source_relative_distance_to_live[name] += _relative_distance(
                    applied, live_reference
                )
        ...
        rows.append(
            {
                ...
                "mean_postburst_source_relative_distance_to_live": 0.0,
            ...
            rows.append(
                {
                    ...
                    "mean_postburst_source_relative_distance_to_live": (
                        source_relative_distance_to_live[name] / count if count else 0.0
                    ),
                    "mean_burst_angle_match_error": angle_match_error[name] / 50.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Hold the post-burst stimulus sequence fixed while comparing live donor-state credit "
            "with credit replayed from the burst-end donor state"
        )
    )
