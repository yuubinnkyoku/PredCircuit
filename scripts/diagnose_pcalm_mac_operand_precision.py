...
class OperandQuantizedResidualMLP(ResidualMLP):
    """ResidualMLP with quantized matrix-product operands and FP32 accumulation.

    Fixed-point quantization uses a straight-through estimator so the forward
    path models quantized operands while weight-credit autograd remains defined.
    This diagnostic deliberately leaves products/accumulation in FP32; it
    ...
    def __init__(
        ...
        *,
        ...
        self.max_abs_operand_pre_quant = 0.0

    def _quantize_ste(self, value: torch.Tensor) -> torch.Tensor:
        quantized, saturated, total, max_pre = quantize_dual(value, self.operand_precision)
        self.operand_saturated += saturated
        self.operand_total += total
        self.max_abs_operand_pre_quant = max(self.max_abs_operand_pre_quant, max_pre)
    ...
    def block_pred(self, layer_ix: int, z_prev: torch.Tensor) -> torch.Tensor:
        ...
        pred = self.scales[layer_ix] * (q_inp @ q_weight.T)
...
def main() -> None:
    ...
    parser.add_argument("--precisions", default="fp32,fp16,bf16,fixed16_i3,fixed12_i3,fixed12_i2")
    ...
    for precision in args.precisions.split(","):
        model = OperandQuantizedResidualMLP(
            depth=depth,
            width=width,
            input_dim=8,
            output_dim=4,
