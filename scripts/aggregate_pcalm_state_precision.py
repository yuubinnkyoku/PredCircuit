...
def main() -> None:
    ...
    summary = (
        frame.groupby("state_precision", sort=False)
        .agg(
            seeds=("seed", "nunique"),
            ...
            gradient_error_to_fp32=("all_gradient_relative_error_to_fp32_state", "mean"),
            residual=("residual_total", "mean"),
            max_abs_state=("max_abs_state_pre_quant", "max"),
            saturation_rate=("state_saturation_rate", "mean"),
        )
        .reset_index()
    )
    a.out.parent.mkdir(parents=True, exist_ok=True)
    frame.to_csv(a.out.with_name("pcalm_state_precision_all.csv"), index=False)
