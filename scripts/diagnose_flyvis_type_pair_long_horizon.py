...
def run_seed(
    *,
    ...
    shuffled_groups = shuffled_groups_like(
        biological_groups,
        edge_count=models["local"].weight.numel(),
                ...
                rows.append(
                    {
                        ...
                        "mean_relative_direction_change": (
                            0.0 if rule == "local" else direction_change_sum[rule] / horizon
                        ),
                        "mean_clip_fraction": clip_sum[rule] / horizon,
                        "mean_weight_norm_error": (
                            0.0 if rule == "local" else weight_error_sum[rule] / horizon
                        ),
                        "mean_bias_error": (
                            0.0 if rule == "local" else bias_error_sum[rule] / horizon
                        ),
                        **metrics,
                        "finite": finite,
                    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Locate emergence of strict norm- and bias-matched type-pair margin effects "
            "at 25/50/75/100 training steps"
        )
    )
