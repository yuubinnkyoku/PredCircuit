...
def apply_update(model: WeightQuantizedResidualMLP, grads: list[torch.Tensor], *, lr: float,
                 stochastic: bool, rng_bits: int, generator: torch.Generator) -> float:
    moved = total = 0
    step = 1.0 / 32.0
    with torch.no_grad():
...
def main() -> None:
    ...
    batches = [(torch.randn(4, 8, generator=data_gen), torch.randn(4, 4, generator=data_gen))
               for _ in range(args.steps)]
    eval_x = torch.randn(16, 8, generator=data_gen)
    eval_y = torch.randn(16, 4, generator=data_gen)
    configs = [("fp32_lr0p01", "fp32", 0.01, False),
    ...
    for name, precision, lr, stochastic in configs:
        model = WeightQuantizedResidualMLP(depth=depth, width=width, input_dim=8, output_dim=4,
            activation="relu", seed=args.seed + depth, weight_precision=precision)
        finite = True
        move_rates = []
        for step_ix, (x, y) in enumerate(batches):
            grads, metrics = run_precision(model, x, y, state_lr=0.25, rho=1.0, alpha=0.925,
                dual_leak=0.01, budget=112, update_precision="fixed12_i2")
            finite = finite and bool(metrics["finite"])
            move = apply_update(model, grads, lr=lr, stochastic=stochastic, rng_bits=12,
                                generator=update_gen)
            move_rates.append(move)
            with torch.no_grad():
                loss = float(torch.mean((stored_forward(model, eval_x) - eval_y) ** 2))
            finite = finite and torch.isfinite(torch.tensor(loss)).item()
            rows.append({"seed": args.seed, "config": name, "step": step_ix + 1,
                         "eval_mse": loss, "weight_move_rate": move, "finite": finite})
